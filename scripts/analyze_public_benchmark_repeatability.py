"""Analyze repeatability across multiple public-benchmark runs."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze repeatability across saved public-benchmark runs."
    )
    parser.add_argument(
        "--run-dir",
        type=Path,
    ...
    parser.add_argument(
        "--results-root",
        type=Path,
        default=DEFAULT_PUBLIC_BENCHMARK_RESULTS_ROOT,
        help="Results root used when discovering runs automatically.",
    )
    args = parser.parse_args()

    analysis = analyze_public_benchmark_repeatability(
        tuple(args.run_dir),
